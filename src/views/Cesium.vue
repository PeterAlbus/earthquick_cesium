  ...
  <el-row ref="viewerContainer" class="demo-viewer">
    ...
    >
      ...
      >
        <vc-entity
          :position="[
            earthquakeInfoList[selectedEarthquakeIndex].longitude,
            earthquakeInfoList[selectedEarthquakeIndex].latitude,
            0,
          ]"
          :description="
            '震源位置:' +
        ...
        >
          <vc-graphics-point
            ...
            color="red"
            :pixelSize="
              2 * earthquakeInfoList[selectedEarthquakeIndex].magnitude
            "
          ></vc-graphics-point>
        </vc-entity>
        <vc-entity
          ...
          :id="
            ...
            ')'
          "
          v-for="item in earthquakeInfoList[selectedEarthquakeIndex]
            .intensityLineList"
        >
          <vc-graphics-ellipse
      ...
      >
        <vc-provider-imagery-tianditu
          mapStyle="cva_w"
          token="125c8a9d540afbc15a4feb04d9c2e8ef"
        ></vc-provider-imagery-tianditu>
      </vc-layer-imagery>
      <vc-layer-imagery
        :alpha="alpha"
    ...
    <el-row class="demo-toolbar">
          ...
          >
            <template #extra>
              <el-button
                type="primary"
                size="small"
                v-if="index !== selectedEarthquakeIndex"
                v-on:click="selectEarthquakeIndex(index)"
                >选择</el-button
              >
              <el-button
                type="info"
                size="small"
                v-if="selectedEarthquakeIndex === index"
                >已选择</el-button
              >
              <el-button
                type="danger"
                size="small"
                v-on:click="deleteEarthquake(item.earthquakeId)"
                >删除</el-button
              >
            </template>
      ...
      <el-button
        type="danger"
        round
        @click="getEarthquakeSituation"
        class="toolbar-item"
        icon="el-icon-document"
        >评估地震情况</el-button
      >
      <el-dialog v-model="dialogVisible" title="地震灾情快速评估" width="30%">
        <p>
      ...
      <el-button
        type="success"
        round
        @click="addEarthquakeVisible = true"
        class="toolbar-item"
        icon="el-icon-plus"
        >添加地震</el-button
      >

      <el-dialog v-model="addEarthquakeVisible" title="添加地震" width="50%">
...
export default {
  ...
  methods: {
    getEarthquakeList() {
      let that = this;
      that.$axios.get("earthquakeInfo/getAllEarthquake").then((res) => {
    ...
    onSubmit() {
      let that = this;
      that.$axios
        .get("earthquakeInfo/addEarthquake?" + that.$qs.stringify(that.form))
        .then((res) => {
          if (res.data === "success") {
            that.$message("添加成功");
    ...
    getEarthquakeSituation() {
      ...
      that.$axios
        .get(
          "earthquakeInfo/getDeathPredict?earthquakeId=" +
            that.earthquakeInfoList[that.selectedEarthquakeIndex].earthquakeId
        )
        .then((res) => {
          that.predict.predictDeath = res.data;
          that.$axios
            .get(
              "earthquakeInfo/getEconomyPredict?earthquakeId=" +
                that.earthquakeInfoList[that.selectedEarthquakeIndex]
                  .earthquakeId
            )
            .then((res) => {
    ...
    deleteEarthquake(id) {
      let that = this;
      that.$axios
        .get("earthquakeInfo/deleteById?earthquakeId=" + id)
        .then((res) => {
          if (res.data === "success") {
            that.getEarthquakeList();
          }
